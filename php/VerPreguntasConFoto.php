...
echo '<style> th {
    ...
    color: white;
}tr:nth-child(even) {background-color: #f2f2f2}</style><table border=1><tr><th> Id </th>><th> Enunciado </th> <th> Respuesta Correcta</th>'
.'<th> Respuestas Incorrecta<s/th>'
.'<th> Complejidad </th><th> Tema </th><th> Imagen </th></tr>';

while($row = mysqli_fetch_array($datos)){
  echo '<tr><td>'.$row['id'].'<tr><td>'.$row['enunciado'].'</td><td>'.$row['respCorrecta'].'</td><td><ul><li>'
